import { find,createAufgabe,findById, update, findUrlaubById, findAufgabeById, findAufgabenDatenById,deleteAufgabe, createTagesZeitKonto, findTagesZeitKontoById, updateTagesZeitKonto,findMonatsArbeitszeitenById } from "../db/queries.js";
...
export const getMonatsArbeitszeiten = async (req, res) => {
  const id = req.params.id;
  try {
    const monatsArbeitszeiten = await findMonatsArbeitszeitenById(id);
    return res.status(200).json( monatsArbeitszeiten );
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal server error" });
  }
